...
afio=$(readlink -f $(dirname $(pwd))/afio)

if [ ! -x "${afio}" ]; then
    printf "error: expected afio binary at ${afio}\n" >&2
    exit 1
fi
...
test_mode () {
    ...
    shift
    # We add “s” (“security”) and “l” (“symlinks”) to the list of ignored
    # warnings; their presence causes afio to terminate non-zero which doesn’t
    # resonate well with a “set -e” shell runtime.
    local cmd=( "${afio}" '-1' mcsl )

    case "${mode}" in
        none)
            # no protection; this is equivalent to
            #   afio -8 allowinsecurepaths -8 allowinsecuresymlinks …
            ;;
        nosymlinks)
            cmd+=( '-8' nosymlinks )
            ;;
